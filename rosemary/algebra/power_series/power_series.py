...
def eta_series(prec=1000):
    ...
    coeff_list = [0] * prec
    coeff_list[0] = 1
    for term, sign in zip(exps(), itertools.cycle((-1, -1, 1, 1))):
        if term >= prec:
            break
...
def ps_mult_karatsuba(M, N, kthresh=40):
    ...
    f1_f0 = [sum(e) for e in itertools.zip_longest(f1, f0, fillvalue=0)]
    g1_g0 = [sum(e) for e in itertools.zip_longest(g1, g0, fillvalue=0)]

    xxyy = ps_mult_karatsuba(f1_f0, g1_g0, kthresh)

    z1 = [e[0] - e[1] - e[2] for e in itertools.zip_longest(xxyy, z2, z0, fillvalue=0)]

    t2 = [0] * (2*k) + z2
    t1 = [0] * (k) + z1
    xy = [sum(e) for e in itertools.zip_longest(t2, t1, z0, fillvalue=0)]
    return xy
